...
2. Regeln berechnen UTC-Zeitpunkte. `create_post` übernimmt eine bewusste Bildwahl des Spiels oder reserviert transaktional ein zufälliges, noch nicht verwendetes Bild aus der ersten zulässigen Medienkategorie der Vereinsrichtlinie. Anschließend löst es die jüngste aktive Promptversion auf, snapshotet Prompt/Design/Seite, erzeugt Text, Feed und alle Storys und legt pro Ausgabe einen `PublicationJob` an.
...
`User`, `UserTeam`, `Team`, `InstagramPage`, `SocialChannelConnection`, `TeamChannelAssignment`, `Game`, `MediaAsset`, `MediaUsageHistory`, `ClubMediaUsagePolicy`, `GameMediaPreference`, `LogoAsset`, `StoryRule`, `PromptTemplate`, `Post`, `PublicationJob`, `AuditLog`, `Notification` und `SystemSetting` bilden das Kernmodell. Eindeutige und zusammengesetzte Constraints verhindern doppelte Spiele, fremde Kanalzuordnungen, Hauptbeiträge, Story-Regeln, Promptversionen, Medienpfade, Logo-Prüfsummen und Idempotency Keys. WhatsApp-Empfänger, Vorlagen, Webhookereignisse und Auslieferungsversuche sind eigenständig und mandantengebunden. Das einmalige `reserved_game_id` schützt eine laufende Auswahl vor konkurrierenden Spielen; `GameMediaPreference` trennt die Auswahl zusätzlich nach Beitragstyp. Beiträge speichern Seite, Design-, Prompt-, Farb-, Font-, Logo-, Medien- und Textversionen als Snapshot.
