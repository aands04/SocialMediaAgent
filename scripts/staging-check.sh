...
check "Testbild technisch lesbar" sh -c "$COMPOSE exec -T web /app/scripts/entrypoint.sh python -c 'from PIL import Image; Image.open(\"/app/external-media/staging_smoke/spieler/smoke-player.png\").verify()'"
check "Path-Traversal blockiert" sh -c "$COMPOSE exec -T web env PYTHONPATH=/app python /app/scripts/check_path_traversal.py"
check "Feed-/Story-/Logo-Upload-Verzeichnisse beschreibbar" sh -c "$COMPOSE exec -T web sh -c 'mkdir -p /app/data/generated/feed /app/data/generated/story /app/data/uploads/logos && touch /app/data/generated/feed/.check /app/data/generated/story/.check /app/data/uploads/logos/.check && rm /app/data/generated/feed/.check /app/data/generated/story/.check /app/data/uploads/logos/.check'"
...
check "DryRun aktiv, Live-Publishing technisch deaktiviert" sh -c "$COMPOSE exec -T web /app/scripts/entrypoint.sh python -c 'from app.config import get_settings; s=get_settings(); assert s.environment==\"staging\" and s.publisher_mode==\"dry-run\" and not s.global_publish_enabled and not s.meta_access_token'"
check "Globaler Not-Aus in Datenbank funktionsfähig" sh -c "$COMPOSE exec -T web /app/scripts/entrypoint.sh python -c 'from app.db import SessionLocal; from app.models import SystemSetting; d=SessionLocal(); x=d.get(SystemSetting,\"emergency_stop\") or SystemSetting(key=\"emergency_stop\",value={\"enabled\":False}); d.add(x); x.value={\"enabled\":True}; d.commit(); assert d.get(SystemSetting,\"emergency_stop\").value[\"enabled\"]; x.value={\"enabled\":False}; d.commit(); d.close()'"
check "FUSSBALL.DE-Live-Modus und Automatik deaktiviert" sh -c "$COMPOSE exec -T web /app/scripts/entrypoint.sh python -c 'from app.config import get_settings; s=get_settings(); assert not s.fussball_live_test_enabled and not s.fussball_automatic_sync_enabled and not s.automatic_post_generation_enabled'"
