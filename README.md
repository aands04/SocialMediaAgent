...
Mit `IMAGE_GENERATOR_MODE=openai` und `TEXT_GENERATOR_MODE=openai` werden Grafiken beziehungsweise Begleittexte über die OpenAI API erzeugt. Standardmodell für Bilder ist `gpt-image-2`. Ausschließlich das Spielerbild wird als generative Bildreferenz übergeben. Vereins- und Gegnerlogos werden der KI niemals als veränderbare Referenz bereitgestellt: Die Anwendung speichert verifizierte PNG-/WebP-Originale versioniert und setzt sie erst nach der KI-Erzeugung mit dem lokalen Compositor reproduzierbar ein. Ohne Gegnerlogo erscheint ein neutraler Text-Fallback; ohne eigenes Mannschaftslogo wird ein KI-Auftrag vor dem kostenpflichtigen Bildaufruf zur manuellen Prüfung gestoppt. Die Ausgabe wird auf exakt 1080 × 1350 (Feed) beziehungsweise 1080 × 1920 (Story) normalisiert und technisch als PNG validiert. Das Modell kann trotz strenger Prompts an anderer Stelle erfundene Wappen, Text oder fehlerhafte Spielermerkmale erzeugen; deshalb bleibt die manuelle visuelle Freigabe zwingend.

Logo-Uploads liegen im persistenten Volume unter `UPLOAD_ROOT=/app/data/uploads`. Das read-only SMB-Verzeichnis wird dabei nie beschrieben. Mannschaftslogos werden unter **Mannschaften**, Gegnerlogos über den Button am jeweiligen Spiel verwaltet. Eine Logoänderung überschreibt keine bestehende Grafik, entzieht offene Freigaben und bietet bei vorhandener KI-Grundgrafik eine rein lokale Neuzusammensetzung ohne weiteren OpenAI-Aufruf. Details: [`docs/VERIFIED_LOGOS.md`](docs/VERIFIED_LOGOS.md).
