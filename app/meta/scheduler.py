...
def _candidate_ids(db: Session, settings: Settings) -> list[tuple[str, str]]:
    ...
    query = (
        ...
        .where(
            ...
            Post.club_id == PublicationJob.club_id,
            InstagramPage.club_id == PublicationJob.club_id,
            InstagramConnection.club_id == PublicationJob.club_id,
            or_(
    ...
    return [(row.id, row.club_id) for row in db.execute(query)]


def _attempt_ids(db: Session, settings: Settings) -> list[tuple[str, str]]:
    now = datetime.now(timezone.utc)
    return [
        (row.id, row.club_id)
        for row in db.execute(
            select(MetaPublishingAttempt.id, MetaPublishingAttempt.club_id)
            .join(Club, Club.id == MetaPublishingAttempt.club_id)
            .where(
...
def _poll_failed_safely(db: Session, attempt_id: str, message: str, settings: Settings) -> None:
    attempt = db.get(MetaPublishingAttempt, attempt_id)
...
def run_automatic_publishing_cycle(
    ...
    *,
    ...
    try:
        with system_scope("Unterbrochene Meta-Aufrufe global abgleichen"):
        ...
        for job_id, club_id in candidates:
            with tenant_scope(club_id, "system:meta-scheduler"):
        ...
        with system_scope("Fällige Meta-Versuche global ermitteln"):
            attempts = _attempt_ids(db, settings)
        for attempt_id, club_id in attempts:
            with tenant_scope(club_id, "system:meta-scheduler"):
                attempt = db.get(MetaPublishingAttempt, attempt_id)
