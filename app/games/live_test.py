...
def serialize(record):
    return {"external_id":record.external_id,"home_team":record.home_team,"away_team":record.away_team,"kickoff":record.kickoff.isoformat(),"kickoff_berlin":record.kickoff.astimezone(BERLIN).isoformat(),"competition":record.competition,"game_number":record.game_number,"source_url":record.source_url,"tracked_team_side":record.tracked_team_side,"status":record.status,"home_score":record.home_score,"away_score":record.away_score,"warnings":list(record.warnings)}
...
def capture(db: Session, team: Team, settings: Settings) -> ProviderSnapshot:
    ...
    try:
        records=provider.parse(html); parsed=[serialize(record) for record in records]; parser_warnings=sorted({warning for record in records for warning in record.warnings})
