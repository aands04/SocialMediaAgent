...
class LiveTestDisabled(RuntimeError):
    pass
...
def capture(db: Session, team: Team, settings: Settings) -> ProviderSnapshot:
    ...
    root = settings.provider_snapshot_root
    root.mkdir(parents=True, exist_ok=True)
    relative = Path(team.id) / f"{now.strftime('%Y%m%dT%H%M%SZ')}-{checksum[:12]}.html"
    ...
    try:
        ...
        parser_warnings = sorted({warning for record in records for warning in record.warnings})
    except ProviderError as exc:
        error = f"Strukturänderung vermutet: {exc}"
    fixture_ids = []
    fixture = Path("tests/fixtures/fussball_sv_ehlen_2627.html")
    if fixture.is_file():
        try:
            fixture_ids = [
                x.external_id for x in provider.parse(fixture.read_text(encoding="utf-8"))
            ]
        except ProviderError:
            pass
    live_ids = [x["external_id"] for x in parsed]
    comparison = {
        "only_live": sorted(set(live_ids) - set(fixture_ids)),
        "only_fixture": sorted(set(fixture_ids) - set(live_ids)),
        "same_count": len(live_ids) == len(fixture_ids),
    }
